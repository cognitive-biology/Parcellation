function [atlas_data,ROI] = cluster2atlas(clusters,atlasnii,atlas_list,regions,varargin)
...
%
% [ATLAS_DATA,ROI] = CLUSTER2ATLAS(CLUSTERS,ATLASNII,ATLAS_LIST,REGIONS) 
...
% region codes REGIONS (1-by-N array). The atlas defined by the path to its 
% nifti file ATLASNII and the list of regions ATLAS_LIST. ATLAS_DATA (2D or 
% 3D matrix) is the mapped version of the clusters and ROI is the list of
% regions in the new atlas.
%
% CLUSTER2ATLAS(CLUSTERS,ATLASNII,ROI,REGIONS,Property1,Value1) initializes
%   property Property1 to Value1.
%   Admissible property:
%       save     -   file name to save ATLAS_DATA and ROI
%       nii      -   file name of the output nifti file
%
...
for ind = 1:length(regions) % number of files should be equal with number of regions
    ...
    for iname = 1:max(cls.Idx)
        s = s+1;
        ROI(s).ID = s;
        ROI(s).Nom_C = strcat(labels(regions(ind)),num2str(iname));
        ROI(s).Nom_L = strcat(names(regions(ind)),'_',num2str(iname));
    end
end
%% saves
...
if nii % save atlas_data as nifti file
    ...
    save_nii(atlas_data,[nii,'.nii'],prop)
end
end
